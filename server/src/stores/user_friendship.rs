...
impl UserFriendshipStore {
    ...
    pub async fn find_for_user_id_requested(
        &self,
        user_id: &Uuid,
    ) -> StoreResult<Vec<user_friendship::Model>> {
        Ok(user_friendship::Entity::find()
            ...
            .filter(
                user_friendship::Column::Status
                    .eq::<i16>(FriendshipStatus::RequestedFrom1.into())
                    .or(user_friendship::Column::Status
                        .eq::<i16>(FriendshipStatus::RequestedFrom2.into())),
            )
            .all(self.db.conn())
            .await?)
    }
    ...
    pub async fn set_status(
        ...
        user_2_id: Uuid,
        status: FriendshipStatus,
    ) -> StoreResult<user_friendship::Model> {
        let friendship =
